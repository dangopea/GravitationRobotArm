#include <Wire.h> //PWM library for sensor servo communication, provides interfacing w I2C devices
#include <Adafruit_PWMServoDriver.h> //compass sensor library 
#include <Adafruit_Sensor.h> //Sensors
#include <Adafruit_LSM303_U.h> //accelerometer and magnetometer libraries

#include "Matrix.h" //matrix and vector implementation
#include "Gyro.h" //MPU gyro sensor implementation

/************************* variables ****************************/

const int servoCount = 6; //number of servos used to control the robotic arm
...
//reference compass vector, calculating the yaw angle of the robotic arm.
Vector compass0;
...
Vector sensors[4];  //vectors of the sensors for readings, has 4 elements- two gyroscopes, an accelerometer, and a magnetometer.
float analog = 0; //last analog reading of the hall effect
float angles[servoCount] = {-1};  //angle values of the joints, corresponding to the six joints of the arm.
float servoValue[servoCount] = {0.5, 0.4, 0.2, 0.25, 0.0, 0.2};   //values the servos will be set to [0; 1]- corresponds to 6 motors
int reversedRotation[servoCount] = {0, 1, 0, 1, 0, 0};  //needed to reverse rotation for servos that are flipped for 6 motors
float offsetValue[servoCount] = {0.5, 0.4, 0.2, 0.25, 0.0, 0.2};  //offset for the angle 0, adjust zero position of servos
...
//pwm driver
Adafruit_PWMServoDriver pwm = Adafruit_PWMServoDriver(0x40); //Object
...
void calibrate() {
  ...
  //calculate the absolute compass vector w sensor readings & rotation matrices, that points directly north- used to calculate yaw angle of arm
  ...
  compass0 = v; //vector will be used to calculate yaw angle of arm
...
void setServoAngle(int servo, float angle) { //Convert angle from radians to a value between 0 & 1, clip value to min & max servo ranges, smooth out servos.
...
void updateServos() { //If servo val is negative, turn it off.
